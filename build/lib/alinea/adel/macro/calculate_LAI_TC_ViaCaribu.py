def calculate_LAI_TC_ViaCaribu(stand_MTG, pattern=None, thermal_time=0, output_directory=''):
    # light string
    ...
    caribu_scene = CaribuScene_nodes.newCaribuScene(stand_MTG, directional_vector, pattern, opt_str)[0]
    # addsoil_2
    ...
    filtered = filterby.filterby(canestra_output_1, canestra_output_2, lambda x: x == 0.0)[0]
    ...
    soil_filtered = filterby.filterby(canestra_output_1, canestra_output_2, lambda x: x == 0.0)[0]
    soil_area = sum(soil_filtered)
    
    canestra_output_1 = selectOutput.selectOutput(energy, 'Opt')[0]
    canestra_output_2 = selectOutput.selectOutput(energy, 'Area')[0]
    canopy_triangles_filtered = filterby.filterby(canestra_output_1, canestra_output_2, lambda x: x > 0.0)[0]
    canopy_triangles_area = sum(canopy_triangles_filtered)
    
    canestra_output_1 = selectOutput.selectOutput(energy, 'Opt')[0]
    canestra_output_2 = selectOutput.selectOutput(energy, 'Area')[0]
    green_triangles_filtered = filterby.filterby(canestra_output_1, canestra_output_2, lambda x: x == 1.0)[0]
    green_triangles_area = sum(green_triangles_filtered)
    
    canestra_output_1 = selectOutput.selectOutput(energy, 'Opak')[0]
    canestra_output_2 = selectOutput.selectOutput(energy, 'Area')[0]
    opak_leaves_filtered = filterby.filterby(canestra_output_1, canestra_output_2, lambda x: x > 0.0)[0]
    opak_leaves_area = sum(opak_leaves_filtered)
    
    canestra_output_1 = selectOutput.selectOutput(energy, 'Opak')[0]
    canestra_output_2 = selectOutput.selectOutput(energy, 'Opt')[0]
    opak_opt_filtered = filterby.filterby(canestra_output_1, canestra_output_2, lambda x: x > 0.0)[0]
    ...
    LAI_PAI_path = write_LAI_PAI.Write_LAI_PAI(thermal_time, efficience, CAI, CAIg, LAI, LAIg, LAI_PAI_path)
